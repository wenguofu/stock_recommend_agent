...
export default function AIDebate() {
  ...
  // 注入markdown-body全局样式（确保所有子元素颜色正确）
  useEffect(() => {
    ...
    if (!document.getElementById(styleId)) {
      ...
      style.textContent = `
        ...
        .dark .markdown-body h4, .dark .markdown-body p, .dark .markdown-body li,
        .dark .markdown-body td, .dark .markdown-body th,
        .dark .markdown-body strong, .dark .markdown-body em,
        .dark .markdown-body a, .dark .markdown-body span,
        .dark .markdown-body div, .dark .markdown-body blockquote,
        .dark .markdown-body code, .dark .markdown-body pre,
        .dark .markdown-body ul, .dark .markdown-body ol { color: #ffffff !important; }
      `;
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">  
        ...
        <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-3 overflow-hidden">  
          {status === 'completed' ? (  
            <div className="h-3 bg-gradient-to-r from-purple-500 to-purple-700 rounded-full" style={{ width: '100%' }} />  
          ) : status === 'failed' || status === 'canceled' ? (  
            <div className="h-3 bg-gradient-to-r from-red-400 to-red-600 rounded-full" style={{ width: '100%' }} />  
          ) : (  
            <>  
              {data?.progress ? (  
                <div   
                  className="h-3 bg-gradient-to-r from-purple-400 to-purple-700 rounded-full transition-all duration-700 ease-out"  
        ...
        <div className="mt-2 flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">  
          {status === 'completed' ? (  
            ...
            <span className="text-red-600 font-medium">❌ 分析失败</span>  
          ) : status === 'canceled' ? (  
            <span className="text-yellow-600 font-medium">⏹️ 已终止</span>  
          ) : status === 'queued' ? (  
            <span>⏳ 排队等待中...</span>  
          ) : (  
            <>  
              {data?.progress ? <span className="font-medium">{data.progress}%</span> : <span>分析中...</span>}  
            </>  
          )}  
          {/* 详细进度 */}  
          {data?.progress_detail?.length ? (  
            <span className="text-xs bg-purple-50 dark:bg-purple-900/20 text-purple-700 dark:text-purple-300 px-2 py-0.5 rounded ml-2">  
              {data.progress_detail[data.progress_detail.length - 1]}  
            </span>  
          ) : null}  
        </div>  
        {/* 历史进度记录 */}  
        {data?.progress_detail?.length ? (  
          <div className="mt-3 max-h-[100px] overflow-y-auto space-y-1">  
            {data.progress_detail.map((d: string, i: number) => (  
              <div key={i} className="flex items-center gap-2 text-xs text-gray-400">  
                {i === data.progress_detail.length - 1 && status === 'running' ? (  
                  <span className="w-2 h-2 bg-purple-500 rounded-full animate-pulse shrink-0"></span>  
                ) : (  
                  <span className="w-2 h-2 bg-green-400 rounded-full shrink-0"></span>  
                )}  
                <span className={i === data.progress_detail.length - 1 && status === 'running' ? 'text-gray-700 dark:text-gray-300 font-medium' : ''}>{d}</span>  
              </div>  
            ))}  
          </div>  
        ) : null}  
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        ...
        {status !== 'completed' ? (
          ...
          <div
            className="markdown-body bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white"
